...
const emptyForm = { orderNo: "", customer: "", company: "", trackingNo: "", status: "PREPARING", notes: "" };

export default function KargoClient() {
  const [cargos, setCargos] = useState<Cargo[]>(initial);
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (editing) {
      setCargos((prev) => prev.map((c) => c.id === editing.id ? { ...c, ...form, updatedAt: new Date().toLocaleDateString("tr-TR") } : c));
    } else {
      setCargos((prev) => [{ id: Date.now().toString(), ...form, updatedAt: new Date().toLocaleDateString("tr-TR") }, ...prev]);
    }
    setModal(false);
  }

  function quickStatus(id: string, status: string) {
    setCargos((prev) => prev.map((c) => c.id === id ? { ...c, status, updatedAt: new Date().toLocaleDateString("tr-TR") } : c));
  }
              ...
              return (
                <tr key={c.id} className={`border-b border-[#f0ebe6] hover:bg-[#faf8f6] ${i === cargos.length - 1 ? "border-b-0" : ""}`}>
                  ...
                  <td className="px-6 py-4">
                    <code className="text-xs bg-[#f5f0eb] px-2 py-1 rounded">{c.trackingNo || "—"}</code>
                  </td>
                  <td className="px-6 py-4">
                    <select
                      value={c.status}
