...
type Record_ = { id: string; type: "INCOME" | "EXPENSE"; description: string; category: string; amount: number; date: string };

const CATEGORIES = {
  INCOME: ["Satış", "Kargo Geliri", "Diğer Gelir"],
  EXPENSE: ["Tedarik", "Lojistik", "Pazarlama", "Kira", "Diğer Gider"],
};
...
const emptyForm = { type: "INCOME" as "INCOME" | "EXPENSE", description: "", category: "", amount: "", date: new Date().toISOString().split("T")[0] };
...
export default function FinansClient() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setRecords((prev) => [{ id: Date.now().toString(), type: form.type, description: form.description, category: form.category, amount: Number(form.amount), date: form.date }, ...prev]);
    setForm(emptyForm);
    setModal(false);
  }
      ...
      <div className="flex gap-2 mb-4">
        {[["", "Tümü"], ["INCOME", "Gelirler"], ["EXPENSE", "Giderler"]].map(([v, l]) => (
          <button
            ...
            className={`text-xs px-4 py-2 rounded-sm border transition-colors ${filterType === v ? "bg-[#2c1810] text-[#f5f0eb] border-[#2c1810]" : "border-[#d4c5ba] text-[#5c4033] hover:bg-[#f5f0eb]"}`}
          >
            {l}
          </button>
        ))}
            ...
            {filtered.map((r, i) => (
              <tr key={r.id} className={`border-b border-[#f0ebe6] hover:bg-[#faf8f6] ${i === filtered.length - 1 ? "border-b-0" : ""}`}>
                <td className="px-6 py-4">
                  <span className={`text-xs px-3 py-1 rounded-full ${r.type === "INCOME" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>
                ...
                <td className={`px-6 py-4 font-medium ${r.type === "INCOME" ? "text-green-700" : "text-red-600"}`}>
