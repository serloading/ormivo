...
export default function MusterilerClient() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (editing) {
      ...
      setCustomers((prev) => [
        ...prev,
        { id: Date.now().toString(), ...form, orders: 0, createdAt: new Date().toLocaleDateString("tr-TR") },
      ]);
    }
    setModal(false);
  }
    ...
    <div>
            ...
            {filtered.length === 0 ? (
              ...
              <tr key={c.id} className={`border-b border-[#f0ebe6] hover:bg-[#faf8f6] ${i === filtered.length - 1 ? "border-b-0" : ""}`}>
                <td className="px-6 py-4">
                  <button onClick={() => setDetailModal(c)} className="font-medium text-[#2c1810] hover:underline text-left">{c.name}</button>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? "Müşteri Düzenle" : "Yeni Müşteri"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <TextareaField label="Not" value={form.note} onChange={(e) => setForm((p) => ({ ...p, note: e.target.value }))} placeholder="VIP müşteri, özel ambalaj ister..." />
          <SubmitRow onCancel={() => setModal(false)} label={editing ? "Güncelle" : "Kaydet"} />
        </form>
      </Modal>

      {/* Detay Modal */}
      <Modal open={!!detailModal} onClose={() => setDetailModal(null)} title="Müşteri Detayı">
        {detailModal && (
          <div className="space-y-4">
            {[
              ["Ad Soyad", detailModal.name],
              ["Telefon", detailModal.phone || "—"],
              ["E-posta", detailModal.email || "—"],
              ["Şehir", detailModal.city || "—"],
              ["Sipariş Sayısı", `${detailModal.orders} sipariş`],
              ["Kayıt Tarihi", detailModal.createdAt],
              ["Not", detailModal.note || "—"],
            ].map(([k, v]) => (
              <div key={k} className="flex justify-between text-sm border-b border-[#f0ebe6] pb-3">
                <span className="text-[#8b6f5e]">{k}</span>
                <span className="text-[#2c1810] font-medium">{v}</span>
              </div>
            ))}
            <a
              href={`https://wa.me/9${detailModal.phone?.replace(/\D/g, "")}`}
              target="_blank"
