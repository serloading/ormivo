...
export default function KuponlarClient({ coupons }: { coupons: Coupon[] }) {
  ...
  function toggleActive(id: string, isActive: boolean) {
    startSave(async () => { await updateCoupon(id, { isActive: !isActive }); router.refresh(); });
  }

  function handleDelete(id: string) {
    if (!confirm("Bu kuponu silmek istediğinizden emin misiniz?")) return;
    startDel(async () => { await deleteCoupon(id); router.refresh(); });
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-white border border-[#e8ddd6] rounded-sm p-6 space-y-4">
