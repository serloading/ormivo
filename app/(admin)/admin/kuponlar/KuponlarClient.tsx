...
import { updateSegmentDiscount } from "@/lib/actions/settings";
...
interface SegmentRates { BRONZE: number; SILVER: number; GOLD: number }

const empty = { code: "", discountType: "PERCENT", discountValue: "", minOrderTotal: "", maxUses: "", expiresAt: "", isActive: true };

const SEGMENT_META = [
  { key: "BRONZE" as const, label: "Bronz Üye",   color: "bg-amber-700 text-white" },
  { key: "SILVER" as const, label: "Gümüş Üye",  color: "bg-slate-400 text-white" },
  { key: "GOLD"   as const, label: "Altın Üye",   color: "bg-yellow-500 text-white" },
];

export default function KuponlarClient({
  coupons, segmentRates: initialRates,
  ...
  // Segment rates — local state for optimistic update
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Segment İndirim Oranları ───────────────── */}
      <div>
        <h2 className="text-2xl font-light tracking-wide text-[#2c1810] mb-1">Segment İndirim Oranları</h2>
        <p className="text-sm text-[#8b6f5e] mb-5">Üye segmentlerine göre ürün fiyatlarına uygulanacak indirim yüzdeleri.</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {SEGMENT_META.map(({ key, label, color }) => (
            <div key={key} className="bg-white border border-[#e8ddd6] rounded-sm p-5">
              ...
              <label className="block text-xs text-[#8b6f5e] mb-1">İndirim Oranı (%)</label>
              <div className="flex items-center gap-2">
                <input
                  ...
                  disabled={rateSaving}
                />
                <span className="text-sm text-[#8b6f5e]">%</span>
                <button
