...
interface Order {
  ...
  trackingNo: string | null; cargoCompany: string | null; note: string | null;
  user: { phone: string; name: string | null } | null;
}

function TrackingForm({ order }: { order: Order }) {
...
export default function SiteSiparislerClient({ orders }: { orders: Order[] }) {
  ...
  return (
    <div className="p-6 max-w-6xl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-gray-800">Site Siparişleri</h1>
              ...
              <div className="flex flex-wrap items-start gap-4 justify-between">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid sm:grid-cols-2 gap-x-6 gap-y-0.5 text-xs text-gray-600 mb-2">
                    ...
                    <span><span className="text-gray-400">Tel:</span> {order.recipientPhone}</span>
                    <span className="sm:col-span-2"><span className="text-gray-400">Adres:</span> {[order.addressLine, order.district, order.city].filter(Boolean).join(", ")}</span>
                    {order.note && <span className="sm:col-span-2"><span className="text-gray-400">Not:</span> {order.note}</span>}
                    {order.user && <span><span className="text-gray-400">Üye:</span> {order.user.name ?? order.user.phone}</span>}
                  </div>
                ...
                <div className="text-right shrink-0">
                  <p className="text-lg font-bold text-gray-800">{Number(order.total).toLocaleString("tr-TR")} ₺</p>
                </div>
              </div>
            </div>
          );
